...
export default function TeachersList() {
  const [teachers] = useState([
    { id: 1, name: "Azizov Alisher", subject: "Matematika", phone: "+998 90 123 45 67", status: "Active" },
    { id: 2, name: "Qodirov Mardon", subject: "Fizika", phone: "+998 90 987 65 43", status: "Active" },
    { id: 3, name: "Usmonova Gulnoza", subject: "Ona tili", phone: "+998 93 111 22 33", status: "Inactive" },
    { id: 4, name: "Toshmatov Vali", subject: "Tarix", phone: "+998 99 444 55 66", status: "Active" },
  ]);
  ...
  return (
    <div className="flex-col gap-6 animate-fade-in" style={{ height: '100%' }}>
      <div className="flex-between">
        <div>
          <h1 className="heading-2">O'qituvchilar qo'mitasi</h1>
          <p className="text-muted">Barcha maktab o'qituvchilari ro'yxati va boshqaruvi</p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--surface-border)', color: 'var(--text-muted)' }}>
                ...
                <th style={{ padding: '1rem' }}>Mutaxassislik Fani</th>
                  ...
                  <td style={{ padding: '1rem' }}>
                    <div className="flex-center gap-3" style={{ justifyContent: 'flex-start' }}>
                      <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'var(--primary)', overflow: 'hidden' }}>
                        <img src={`https://ui-avatars.com/api/?name=${teacher.name}&background=6366f1&color=fff`} alt="" style={{width:'100%', height:'100%'}} />
                      </div>
                      <span style={{ fontWeight: 500 }}>{teacher.name}</span>
                    </div>
                  </td>
                  <td style={{ padding: '1rem', color: 'var(--text-muted)' }}>{teacher.subject}</td>
